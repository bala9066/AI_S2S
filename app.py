...
def _sanitize_mermaid(code: str) -> str:
    """Fix common Mermaid syntax issues generated by LLMs (compatible with v10)."""
    ...
    # Mermaid v10 doesn't support quoted labels in subgraph with bracket syntax
    code = re.sub(r'subgraph\s+(\w+)\s*\["([^"]+)"\]', r'subgraph \1[\2]', code)

    # Fix node labels with special chars that break parser: replace & with and
    code = re.sub(r'\[([^\]]*?)&([^\]]*?)\]', lambda m: '[' + m.group(1) + 'and' + m.group(2) + ']', code)

    # Fix arrows with text that include colons (use | | instead)
    # e.g.  A -->|"label: text"| B  →  A -->|label text| B
    code = re.sub(r'\|"([^"]+)"\|', lambda m: '|' + m.group(1).replace(':', ' -') + '|', code)

    # Remove any lines with just comments or empty
