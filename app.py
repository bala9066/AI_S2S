...
def _update_phase_status(pid: str, status: str, extra: dict = None):
    if "current_project" in st.session_state:
        ...
        # Persist to database so phase status survives tab navigation
        try:
            from database.models import get_session, ProjectDB
            proj_id = st.session_state.current_project.get("id")
            if proj_id:
                db_session = get_session()
                db_proj = db_session.query(ProjectDB).filter(ProjectDB.id == proj_id).first()
                if db_proj:
                    db_proj.phase_statuses = dict(statuses)
                    if status == "completed" and pid == "P1":
                        db_proj.current_phase = "P2"
                    db_session.commit()
                db_session.close()
        except Exception as e:
            pass  # Non-fatal: session state still has the update


def _sanitize_mermaid(code: str) -> str:
    """Fix common Mermaid syntax issues generated by LLMs (compatible with v10.2.4).

    Root causes found via browser console:
    - Unicode chars like -> (arrow), +/- break the Mermaid v10 lexer
    - subgraph labels with quotes need special handling
    """
    ...
    # Replace Unicode characters that break Mermaid v10 lexer
    unicode_fixes = [
        ('\u2192', '->'),    # → right arrow
        ('\u2190', '<-'),    # <- left arrow
        ('\u2194', '<->'),   # <-> bidirectional
        ('\u00b1', '+/-'),   # ± plus-minus (CONFIRMED breaking)
        ('\u00d7', 'x'),     # × multiplication
        ('\u00f7', '/'),     # ÷ division
        ('\u00b0', 'deg'),   # ° degree
        ('\u00b5', 'u'),     # µ micro
    ...
    # Fix subgraph labels: subgraph X["Label"] -> subgraph X[Label]
    code = re.sub(r'subgraph\s+(\w+)\s*\["([^"]+)"\]', r'subgraph \1[\2]', code)

    # Fix node labels with & -> replace with 'and'
    code = re.sub(r'\[([^\]]*?)&([^\]]*?)\]', lambda m: '[' + m.group(1) + 'and' + m.group(2) + ']', code)

    # Fix quoted arrow labels with colons: |"label: text"| -> |label text|
...
def render_pipeline():
    ...
    # Sync phase_statuses from DB to get latest state (survives tab navigation)
    try:
        from database.models import get_session, ProjectDB
        db_sess = get_session()
        db_proj = db_sess.query(ProjectDB).filter(ProjectDB.id == proj.get("id")).first()
        if db_proj and db_proj.phase_statuses:
            proj["phase_statuses"] = db_proj.phase_statuses
