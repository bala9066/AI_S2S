...
class CodeAgent(BaseAgent):
    ...
    def _validate_ci_workflow(self, yaml_content: str) -> str:
        ...
        lines.append('git commit -m "[AI] Hardware Pipeline: add CI/CD workflow"')
...
jobs:
  ...
  build-firmware-arm:
    ...
    steps:
      ...
      - name: Build firmware
        ...
        run: make CROSS_COMPILE=arm-none-eabi- all || make -f Makefile all || true
  ...
  unit-tests:
    ...
    steps:
      ...
      - name: Run unit tests
        working-directory: tests
        run: ./run_tests --gtest_output=xml:test-results.xml || true
