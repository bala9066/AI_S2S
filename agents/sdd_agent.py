...
class SDDAgent(BaseAgent):
    ...
    async def execute(self, project_context: dict, user_input: str) -> dict:
        ...
        user_message = f"""Generate a complete IEEE 1016 SDD for:
...
Generate the complete SDD with:
- All IEEE 1016 viewpoints
- Mermaid diagrams for each viewpoint (class, sequence, state, component)
- C/C++ struct definitions for register maps
- Function prototypes for all driver APIs
- Error handling architecture
- Traceability to REQ-SW-xxx
"""
        ...
        sdd_file = output_dir / f"SDD_{project_name.replace(' ', '_')}.md"
        sdd_file.write_text(sdd_content, encoding="utf-8")
